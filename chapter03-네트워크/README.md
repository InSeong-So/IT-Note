...
- `1 Layer : Physical` *(물리 계층)*
  - 실제 장치들을 연결하기 위해 필요한 전기적, 물리적 세부 사항들을 정의합니다.
  
  - 전선, 전파, 광섬유, 동축케이블, 도파관, PSTN, Repeater, DSU, CSU, Modem

- `2 Layer : Data link` *(데이터 링크 계층)*
  - 포인트 투 포인트(Point to Point) 간 신뢰성 있는 전송을 보장합니다.
...
- `4 Layer : Transport` *(전송 계층)*
  - 양 끝단(End to end)의 사용자들이 신뢰성있는 데이터를 주고 받을 수 있도록 해줍니다. 헤드에는 세그먼트가 포함되며 주소 설정, 오류 및 흐름 제어, 다중화를 수행합니다.
