...
> 운영체제의 핵심 기능을 모아놓은 커널(kernel), 사용자와 응용 프로그램에 인접해 커널에 명령을 전달하고 실행 결과를 돌려주는 인터페이스(interface)에 대해 알아봅니다.
...
| 영역        | 설명                                                                                                       |
| ----------- | ---------------------------------------------------------------------------------------------------------- |
| `코드` 영역   | 실행될 프로그램의 코드가 저장                                                                              |
| `데이터` 영역 | 전역 변수와 정적 변수가 저장                                                                               |
| `스택` 영역   | 지역변수와 매개 변수가 저장되어 있으며, 함수의 호출과 함께 할당<br>컴파일 타임(Compile time)에 크기가 결정 |
| `힙` 영역     | 사용자에 의해 동적으로 할당되고 해제될 수 있는 메모리 영역<br>런 타임(Run time)에 크기가 결정              |
...
## :book: `Process` 와 `Thread` 의 차이점을 알려주세요.
### Process 의 개요
...
## :book: `Multi-Process` 와 `Multi-Thread` 에 대해 알려주세요.
### Multi-Process
...
## :book: `User Thread` 와 `Kernel Thread` 의 차이점은 무엇인가요?
...
## :book: `Context-Switching` 이 무엇인지 설명해주세요.
...
## :book: 단편화(Fragmentation) 란 무엇인가요?
...
## :book: `Paging` 과 `Segmentation` 의 차이점에 대해 알려주세요.
### `Paging` 의 개요
...
## :book: `CPU Scheduling` 에 대해 알려주세요.
### CPU 스케줄링의 개요
...
## :book: `Sync` 와 `Async` 의 차이점을 알려주세요.
...
## :book: `Virtual Memory` 에 대해 알려주세요.
